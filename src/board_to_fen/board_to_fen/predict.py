...
from board_to_fen.board_to_fen import saved_models
...
def get_fen_from_image_path(image_path, end_of_row='/', black_view=False) -> str:
    ...
    f = pkg_resources.open_text(saved_models, 'november_model_weights.h5')
...
def get_fen_from_image(image, end_of_row='/', black_view=False) -> str:
    ...
    # net.model.built = True
    # net.load_model(path='src/board_to_fen/board_to_fen/saved_models/november_model_weights.h5')
    # net.load_model(path='src/board_to_fen/board_to_fen/saved_models')
    net.model.load_weights('src/board_to_fen/board_to_fen/saved_models/november_model_weights.h5')
    tiler = Tiler()
    tiles = tiler.get_tiles(img=image)
    predictions = net.predict(tiles=tiles)
    fen = decoder.fen_decode(squares=predictions, end_of_row=end_of_row, black_view=black_view)
